...
- Data tables of every kind: stock and commodity prices, exchange rates,
  market summaries, sports scorecards, points tables, fixtures and results,
  weather tables, or any other financial/statistical table, whether
  standalone or embedded in a story. Do not transcribe or explain them.
...
If a whole page turns out to have no article, reply with the single line
NO ARTICLES ON THIS PAGE and nothing else. That is a correct and useful
...
OUTPUT FORMAT
Reply with ONLY article blocks in exactly this shape. No preamble, no
closing remarks, no markdown headings other than the ### markers.
...
# page holds nothing but adverts, classifieds, listings or tables. Matched
# loosely, because models paraphrase the sentinel rather than echoing it
# exactly.
...
def parse_response(raw: str) -> ParseResult:
    ...
    # listings or tables. It is a correct answer, so it must not be treated
    # as a failure.
    upper = text.upper()
    if _NO_ARTICLES.search(text) and "### ARTICLE" not in upper:
    ...
    if not result.articles:
        if saw_any_marker:
            result.warnings.append(
                "found ### ARTICLE markers but none had both a HEADLINE and "
                "a SUMMARY line"
            )
        else:
            result.warnings.append(
